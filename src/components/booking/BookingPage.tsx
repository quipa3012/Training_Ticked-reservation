...
export default function BookingPage({ movieId }: Props) {
  ...
  const availableShowtimes = showtimes.filter(
    (s) => s.movieId === movieId && (!cinemaId || s.cinemaId === cinemaId)
  );

  // ghế đã đặt theo showtime
  const bookedSeats = bookings.filter((b) => b.showtimeId === showtimeId).flatMap((b) => b.seats);
    ...
    <div className={styles.container}>
      <Card>
        ...
        <div className={styles.section}>
          ...
          <Select
            ...
            options={availableShowtimes.map((s) => ({
              value: s.id,
              label: `${dayjs(s.startTime).format('DD/MM HH:mm')} – ${s.price.toLocaleString()} đ`,
            }))}
          />
        </div>
        ...
        <div className={styles.footer}>
          <div>Ghế đã chọn: {selectedSeats.join(', ') || '—'}</div>
          <div>
